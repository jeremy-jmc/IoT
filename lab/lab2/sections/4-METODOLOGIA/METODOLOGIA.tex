...
\begin{figure}[H]
    ...
    \caption{Simulación Tinkercad para el control de un motor con Driver}
...
Si el nivel de humedad supera el 75\%, el sistema considera que la planta está sobre-hidratada. En este caso, se activa una alarma sonora (buzzer) y visual (LED rojo intermitente), ambos operando de forma parpadeante para llamar la atención. La Figura~\ref{fig:riego_sobrehidratacion} muestra este estado en la implementación. El funcionamiento general del sistema puede observarse en las Figuras~\ref{fig:riego_regando} (estado de riego), \ref{fig:riego_sobrehidratacion} (estado de alerta por sobre-hidratación) y \ref{fig:riego_logs} (logs del sistema), mientras que la arquitectura general se resume en el diagrama de bloques de la Figura~\ref{fig:bloques_riego_inteligente}. El código completo que implementa la lógica descrita se presenta en el Código~\ref{code:riego_inteligente}. Finalmente, el diagrama de flujo del sistema se presenta en la Figura~\ref{fig:riego_diagrama_flujo}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{./img/12-Diagrama-Flujo.png}
    \caption{Diagrama de flujo del sistema de riego inteligente}
    \label{fig:riego_diagrama_flujo}
\end{figure}


\section{ACTUADORES - Control de ventiladores}
Para este laboratorio se deseo implementar un sistema de dos ventiladores que se controlaran mediante dos sensores de temperatura. El sistema se implemento en Tinkercad, utilizando un Arduino UNO y dos sensores DHT11. El objetivo del sistema es encender el ventilador 1 cuando la temperatura es mayor a 28 grados en el sensor 1 y en el sensor 2 sea menor a 28, caso contrario prender el ventilador 2. La simulacion se puede observar en la Figura \ref{fig:ventiladores}. El diagrama de bloques del sistema se puede observar en la Figura \ref{fig:bloques_ventiladores} y el codigo en el Código \ref{code:ventiladores}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{./img/13-Controlador-Ventiladores.png}
    \caption{Simulación Tinkercad Ventiladores}
    \label{fig:ventiladores}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{./img/13-Diagrama-Bloques.png}
    \caption{Diagrama de bloques del sistema de ventiladores}
    \label{fig:bloques_ventiladores}
\end{figure}

\begin{lstlisting}[style=cppstyle, caption={Código en C++ para el sistema de ventiladores.}, label={code:ventiladores}]
...
void setup() {
  ...
  digitalWrite(relePin, LOW); // Rele apagado al inicio
}

void loop() {
  float t1 = ((analogRead(sensor1) * 5.0 / 1023.0) - 0.5) * 100.0;
  float t2 = ((analogRead(sensor2) * 5.0 / 1023.0) - 0.5) * 100.0;
  float rele = digitalRead(relePin);
  Serial.print("Temp1: "); Serial.print(t1);Serial.print(" | D2: "); Serial.print(rele) ;Serial.print(" | Temp2: "); Serial.println(t2);
  
  if (t1 > umbral && t2 <= umbral) {
    digitalWrite(relePin, LOW);  
  } else if (t2 > umbral && t1 <= umbral) {
